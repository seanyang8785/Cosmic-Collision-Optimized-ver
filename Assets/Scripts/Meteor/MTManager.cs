...
public class MTManager : MonoBehaviour
{
    ...
    void Start() {
        if(SceneManager.GetActiveScene().buildIndex == 1 && GameObject.FindGameObjectWithTag("Meteor") == null){
            ...
            // MTPrefab.GetComponentInChildren<SpriteRenderer>()[0]
    ...
    void Update()
    {
        if(SceneManager.GetActiveScene().buildIndex == 1 && GameObject.FindGameObjectWithTag("Meteor") == null && GameObject.FindGameObjectWithTag("No.1") != null){
            if(PauseBackTo.restarted){
                GameManager.difficultyUpdate();
                PauseBackTo.restarted = false;
            }
            else{
                amount += 1;
            }
